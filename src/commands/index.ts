...
export async function configureGemini(context: vscode.ExtensionContext) {
  const apiKey = await vscode.window.showInputBox({
...
export async function generateDailyStandup(context: vscode.ExtensionContext) {
  try {
...
export async function configureSlack(context: vscode.ExtensionContext) {
  const config = vscode.workspace.getConfiguration("daily-standup-bot");
...
export async function testSlackConnection(context: vscode.ExtensionContext) {
  try {
    const config = vscode.workspace.getConfiguration("daily-standup-bot");
